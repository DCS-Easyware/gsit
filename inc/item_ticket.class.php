   ...
   **/
   static function showForTicket(Ticket $ticket) {
      ...
      for ($i=0 ; $i<$number ; $i++) {
         ...
         if ($itemtype == 'PluginAppliancesAppliance_Item') {
            if (in_array($itemtype, $_SESSION["glpiactiveprofile"]["helpdesk_item_type"])) {
               $itemtable = getTableForItemType($itemtype);
               $query = "SELECT `$itemtable`.*,
                                `glpi_items_tickets`.`id` AS IDD,
                                `glpi_plugin_appliances_appliances`.`id` as appid,
                                `glpi_plugin_appliances_appliances`.`name` as appname
                         FROM `glpi_items_tickets`,
                              `$itemtable`";
               $query .= "LEFT JOIN `glpi_plugin_appliances_appliances`"
               ...
               for ($prem=true ; $data=$DB->fetch_assoc($result_linked) ; $prem=false) {
                  ...
                  if($_SESSION['glpiactiveprofile']['interface'] != 'helpdesk') {
                     $link     = Toolbox::getItemTypeFormURL('PluginAppliancesAppliance');
        ...
        } else if ($itemtype == 'PluginDatabasesDatabase_Item') {
            if (in_array($itemtype, $_SESSION["glpiactiveprofile"]["helpdesk_item_type"])) {
               $itemtable = getTableForItemType($itemtype);
               $query = "SELECT `$itemtable`.*,
                                `glpi_items_tickets`.`id` AS IDD,
