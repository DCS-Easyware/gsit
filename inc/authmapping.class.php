...
class AuthMapping extends CommonDBTM {
   ...
   **/
   ...
   function manageAttributes($userdata, $attributes) {
      global $DB;

      $iterator = $DB->request([
         'SELECT' => ['remotefield', 'userfield'],
         'FROM'   => $this->getTable(),
//         'WHERE'  => ['computers_id' => $this->getID()]
      ]);
      while ($data = $iterator->next()) {
         if (isset($attributes[$data['remotefield']])
               && !isset($userdata[$data['userfield']])) {
            $userdata[$data['userfield']] = $attributes[$data['remotefield']][0];
         }
      }
      // if user account exist, update data
      $user = new User();
      if ($user->getFromDBbyName($userdata['name'])) {
         $userdata_update = $userdata;
         $userdata_update['id'] = $user->fields['id'];
